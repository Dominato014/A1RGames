;window.CloudflareApps=window.CloudflareApps||{};CloudflareApps.siteId="77681d9e444a435e0d96be553bd89126";CloudflareApps.installs=CloudflareApps.installs||{};;(function(){'use strict'
...
return false}}());CloudflareApps.installs["VSxKWEL7WfLb"]={appId:"mt9QUlJFKGSg",scope:{}};;CloudflareApps.installs["VSxKWEL7WfLb"].options={"account":{"accountId":"l-xCPNH2t12G","service":"jsguardian"},"free":{"guardInnerHTML":"strict","guardInput":"strict","guardPrompt":"clean","guardTransport":"https-only","guardWhen":"before","guardXSS":"block"}};;if(CloudflareApps.matchPage(CloudflareApps.installs['VSxKWEL7WfLb'].URLPatterns)){(function(){"use strict";function cloneObject(obj,done=new Map()){if(done.has(obj)){return done.get(obj);}
...
return"";}
var _addEventListener=window.addEventListener,_setAttribute=Element.prototype.setAttribute,options=cloneObject(typeof(CloudflareApps.installs['VSxKWEL7WfLb'].options)!=="undefined"?CloudflareApps.installs['VSxKWEL7WfLb'].options:{"free":{"guardWhen":"before","guardInnerHTML":"clean","guardPrompt":"clean","guardInput":"clean","guardTransport":"https-only","guardXSS":"sanitize"},"plan":"free"});function isExecutable(str){var functionExp=/([\s\(]*.\(\s*.*\s*\)\s*.*\s*=>)|([\s*.*\s*=>\s*.\{)|([\s\(]*.[Ff]unction\s*.*\s*\(\s*.*\s*\)\s*.*\s*\{)/,templateExp=/(\`(\s*.*)\$\{(\s*.*\s*.*)\}(\s*.*)\`)/;return functionExp.test(str)||templateExp.test(str)};function protect(){if(options.guardInput&&options.guardInput!=="off"){(function(){if(_addEventListener){_addEventListener.call(window,"change",function(event){var target=event.currentTarget;if(target instanceof HTMLInputElement||target instanceof HTMLTextAreaElement){var oldvalue=target.getAttribute("value"),clean=sanitize(target.value);if(oldvalue===clean){return;}
